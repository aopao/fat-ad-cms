...
class Buiapi extends Backend
{
	 ...
	 * 修改字段状态
	 */
	public function tablefieldedit(){
		$this->request->filter('trim,strip_tags,htmlspecialchars');
		$field  = "";
		$params = $this->request->get("","", 'trim');
		$value  = empty($params['value']) ? 1 : 0;
		if(empty($params['id']) || !in_array($params['field'],['is_byadd','is_byedit'])){
			return json(['code' => 1, 'msg' => '参数错误']);
		}
		if(in_array($params['field'],['is_byadd','is_byedit'])){
			$field = trim($params['field']);
		}
		$result = $this->model_field->where(['id'=>$params['id']])->update(["{$field}"=>$value]);
		if(empty($result)){
			return json(['code' => 1, 'msg' => '失败']);
		}		
		return json(['code' => 0, 'msg' => '成功','value'=>$value]);
	}
    ...
    public function index()
    {
		...
		$this->view->assign('is_show', $config['isshow_apidoc']);
	 ...
	 * 处理命令行
	 */
	protected function __handle_command__(){
		$commond_table = "";
		$commond_array = explode('--',$this->request->request('commond'));
		foreach($commond_array as $fkey=>$fval){
			if(!empty($fkey)){
				list($field,$value) = explode("=",$fval);
				if($field == "table"){
					$commond_table = $value;
				}
			}
		}
		if(!empty($commond_table)){
			Db("buiapi_table")->where(["table"=>$commond_table])->update(['command'=>$this->request->request('commond')]);
		}
	}
}
